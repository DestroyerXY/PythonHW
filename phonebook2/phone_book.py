class Contact:
    ...
    def __str__(self):
        return f'{self.name:<20} | {self.phone:<20} | {self.comment:<20}' #распечатывает контакт
...
class PhoneBook:
    ...
    def __str__(self):
        ...
        for contact in self.phone_list:
            i += 1
            result += f'{i}. {contact}\n'
        return result[:-2] #тел книга склеивает

    def replace_contact(self, index_1: int, index_2: int):
        self.phone_list.insert(index_2, self.phone_list.pop(index_1))
